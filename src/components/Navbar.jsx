      ...
      <ul className="hidden md:flex">
        {links.map(({ id, link }) => {
          return (
            <li
              key={id}
              className="px-4 cursor-pointer capitalize font-medium text-gray-400 hover:scale-105 duration-100 hover:text-yellow-200"
            >
        ...
        <ul className="flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-black to-gray-800 text-gray-400">
          {links.map(({ id, link }) => {
            return (
              <li
                key={id}
                className="py-4 px-4 text-4xl cursor-pointer capitalize font-medium text-gray-400 hover:scale-105 duration-100 hover:text-yellow-200 "
              >
                <Link
                  onClick={() => {
                    setHambIcon(!hambIcon);
